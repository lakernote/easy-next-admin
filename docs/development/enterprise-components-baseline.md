...
| 业务支撑能力 | 业务编号、流程单号、导入导出规范、通知模板、业务事件、批处理 | 部分具备 | 缺统一业务编号生成器、消息模板、批处理治理、业务事件模型 | P1 |
...
- 真正缺的是生产化闭环：交付包、发布回滚、增长治理、结构化日志、告警面板、Outbox 死信、业务编号、安全会话和灾备恢复。
...
| 业务编号 | 申请单号、工单号、采购单号、按日流水号、可读短码 | 缺口 | 表主键使用 `ASSIGN_ID`，业务编号未统一 | P1：新增应用内 `BusinessSequenceService`，支持前缀、日期、流水和分布式并发安全 |
...
| 批处理治理 | 长任务进度、断点续跑、取消、重试、结果文件、失败明细 | 缺口 | 当前有定时任务和用户导入导出，但缺统一批处理模型 | P1：用于大批量导入、批量同步、批量生成报表等耗时任务 |
...
| P1 | 业务编号生成器 | 分布式主键已够用，但工单号、申请单号等可读编号还缺统一应用组件 | `BusinessSequenceService`、MySQL 号段或 Redis 原子递增、编号规则文档 |
...
| P1 | 批处理治理模型 | 大批量导入、同步和报表生成不能只靠同步接口或普通定时任务 | 批处理任务表、进度、失败明细、取消、重试、结果文件 |
