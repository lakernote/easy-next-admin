...
public class MysqlEasyLocker extends AbstractSimpleIEasyLocker {
     ...
     */
    public static final String ACQUIRE_FORMATTED_QUERY = "INSERT ignore INTO infra_distributed_lock (lock_key, token, expire, thread_id) VALUES (?, ?, ?, ?);";
    public static final String RELEASE_FORMATTED_QUERY = "DELETE FROM infra_distributed_lock WHERE lock_key = ? AND token = ?;";
    public static final String DELETE_EXPIRED_FORMATTED_QUERY = "DELETE FROM infra_distributed_lock WHERE expire < ?;";
    public static final String REFRESH_FORMATTED_QUERY = "UPDATE infra_distributed_lock SET expire = ? WHERE lock_key = ? AND token = ?;";
    ...
    public boolean release0(EasyLocker lock) {
        ...
        if (released) {
            log.debug("Release query successfully affected 1 record for key {} with token {}", key, token);
        } else if (deleted > 0) {
