package com.laker.admin.infrastructure.message.local.mapper;
...
public interface LocalMessageMapper extends BaseMapper<LocalMessage> {
    ...
    // 按时间查询最近10条消息
    @Select("SELECT * FROM infra_local_message WHERE status = #{status} AND deleted = 0 ORDER BY create_time DESC LIMIT 10")
    ...
    @Update("UPDATE infra_local_message SET process_tag = #{processTag} " +
            "WHERE id IN (SELECT id FROM (SELECT id FROM infra_local_message WHERE status = #{status} " +          // for update skip locked
            "   AND deleted = 0 AND process_tag IS NULL ORDER BY create_time DESC LIMIT 10 for update skip locked ) tmp) " + //  查询返回查询结果，但忽略有行锁的记录
            "   AND status = #{status} AND deleted = 0 AND process_tag IS NULL")
    int updateProcessTagByStatus(@Param("status") String status, @Param("processTag") String processTag);

    // 根据 process_tag 查询更新后的记录
    @Select("SELECT * FROM infra_local_message WHERE process_tag = #{processTag} AND deleted = 0")
