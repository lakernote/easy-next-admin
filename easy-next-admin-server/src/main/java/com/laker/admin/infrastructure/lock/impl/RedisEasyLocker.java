...
//        final boolean locked = stringRedisTemplate.execute(lockScript, singletonKeyList, token, String.valueOf(expiration));
    ...
    protected boolean release0(EasyLocker lock) {
        ...
        if (released) {
            log.debug("Release script deleted the record for key {} with token {}", key, token);
        } else {
            log.warn("Release script failed for key {} with token {}", key, token);
        }
        return released;
    }
    ...
    protected boolean refresh(final String key, final String token, final Duration expiration) {
        ...
        try {
            ...
            if (refreshed) {
                log.debug("Refresh script updated the expiration for key {} with token {} to {}", key, token, expiration);
            } else {
                log.warn("Refresh script failed to update expiration for key {} with token {} with expiration: {}", key, token, expiration);
            }
