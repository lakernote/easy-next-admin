 ...
 */
@Slf4j
public class EasyNextAdminThreadPoolExecutor extends ThreadPoolExecutor {
    /**
     * 简易线程池构造器
     *
     ...
     */
    public EasyNextAdminThreadPoolExecutor(int poolSize, int queueSize, String prefix) {
        super(poolSize, poolSize, 60, TimeUnit.SECONDS,
                new LinkedBlockingQueue<>(queueSize), namedThreadFactory(prefix + "-"),
                new EasyNextAdminRejectPolicy());
    ...
    public EasyNextAdminThreadPoolExecutor(int corePoolSize, int maximumPoolSize,
                                       long keepAliveTime, TimeUnit unit,
                                       BlockingQueue<Runnable> workQueue, ThreadFactory threadFactory) {
        super(corePoolSize, maximumPoolSize, keepAliveTime, unit, workQueue,
                threadFactory, new EasyNextAdminRejectPolicy());
        // 非核心线程如果处于闲置状态超过该值，就会被销毁。如果设置allowCoreThreadTimeOut(true)，则会也作用于核心线程。
        this.allowCoreThreadTimeOut(true);
    }
