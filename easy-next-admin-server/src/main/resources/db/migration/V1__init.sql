...
CREATE TABLE `sys_menu` (
  ...
  `power_code` varchar(50) DEFAULT NULL COMMENT '菜单或按钮权限标识',
  ...
  KEY `idx_sys_menu_code_deleted` (`power_code`,`deleted`),
...
CREATE TABLE `sys_role_permission` (
  ...
  `power_id` bigint NOT NULL COMMENT '权限资源ID',
  `create_by` bigint DEFAULT NULL COMMENT '创建人ID',
  `create_dept_id` bigint DEFAULT NULL COMMENT '创建部门ID',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_by` bigint DEFAULT NULL COMMENT '更新人ID',
  ...
  `remark` varchar(500) DEFAULT NULL COMMENT '备注',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `uk_sys_role_permission_role_power` (`role_id`,`power_id`),
  KEY `idx_sys_role_permission_role` (`role_id`,`deleted`),
  KEY `idx_sys_role_permission_power` (`power_id`,`deleted`),
